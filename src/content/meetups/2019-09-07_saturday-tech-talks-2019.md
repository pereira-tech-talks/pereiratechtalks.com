---
title:
  en: "Pereira - Saturday Tech Talks"
  es: "Pereira - Saturday Tech Talks"
description:
  en: "Pereira Tech Talks meetup — Pereira - Saturday Tech Talks. Community archive page with the night’s program and context."
  es: "📣 ¡Un gran evento organizado por todas las comunidades de tecnología en Pereira nos espera! Tendremos unas charlas buenísimas que no te puedes perder:"
pubDate: 2019-09-07
date: 2019-09-07
venue:
  name: "Sala Magistral 1 (Edificio 13)"
  city: "Pereira"
  country: "Colombia"
mode: in-person
hero:
  src: "/images/meetups/saturday-tech-talks-2019/hero.jpeg"
  alt:
    en: "Pereira - Saturday Tech Talks"
    es: "Pereira - Saturday Tech Talks"
  layout: banner
heroImage: "/images/meetups/saturday-tech-talks-2019/hero.jpeg"
verticals:
  - monthly-meetups
talks:
  - saturday-tech-talks-2019--1-desarrollo-serverless-de-interfaces-conversacionales-en
  - saturday-tech-talks-2019--2-andres-rodriguez-session
  - saturday-tech-talks-2019--3-introduccion-a-amazon-web-services-aws
  - saturday-tech-talks-2019--4-patrones-de-arquitectura-android
speakers:
  - andres-rodriguez
  - steven-pineda
  - zorayda-gutierrez
  - sergio-florez
sponsors: []
status: completed
draft: false
---

## Pereira - Saturday Tech Talks

📣 ¡Un gran evento organizado por todas las comunidades de tecnología en Pereira nos espera! Tendremos unas charlas buenísimas que no te puedes perder:

🔎 Next.js es una herramienta que te permite hacer una aplicación web con React renderizando desde el lado del Server. Veremos las ventajas que hay cuando renderizas desde el lado del servidor, versus las ventajas de redenrizar desde el lado del cliente (Browsers) y como Next.js puede convivir con estos dos tipos de ventajas.

### Charlas

1. **Next.js: Server-side rendering your React App** — Andrés Rodriguez Escudero ([perfil](https://twitter.com/arodrigueze_))
2. **Introducción a Amazon Web Services (AWS)** — Steven Pineda Cortes ([perfil](https://twitter.com/Steven2308))
3. **Patrones de arquitectura - Android** — Zorayda Gutiérrez Montes ([perfil](https://www.facebook.com/zoraidayadi.gutierrezmontes))
4. **Desarrollo serverless de interfaces conversacionales en Amazon Alexa y Google Home** — Sergio Alexander Florez Galeano

---

### Fuentes / Sources

- Original event page: [Meetup.com](https://www.meetup.com/pereira-tech-talks/events/264304830)
- Seguimos recuperando fotos, enlaces a slides y grabaciones desde los archivos de la comunidad — los PR son bienvenidos.
