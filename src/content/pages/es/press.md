---
title: "Prensa — Pereira Tech Talks"
description: "Kit de prensa de Pereira Tech Talks: hechos clave, boilerplate, descargas de marca y contacto para periodistas y medios."
lastUpdated: 2026-08-02
---

## Para medios

Comunidad sin fines de lucro en Pereira, Risaralda. Activa desde 2014. Esta página resume hechos clave, un boilerplate listo para publicar, descargas de marca y el contacto de prensa.

---

## Hechos clave

- **Fundación:** Pereira, Risaralda, Colombia · 2014
- **Naturaleza:** Comunidad sin fines de lucro, voluntariado
- **Idiomas:** Sitio en español e inglés
- **Programas:** Meetups · Pereira Tech Day · Escuela de Speakers · La Biblioteca del Mañana · Canal de IA
- **Cifras:** 90+ meetups · 200+ charlas · 6.500+ asistencias · 7+ ediciones PTD
- **Modelo:** Eventos gratuitos sostenidos por patrocinio

---

## Boilerplate

Pereira Tech Talks es una comunidad de constructores, ponentes y aprendices con sede en Pereira, Risaralda, Colombia. Desde 2014 hemos organizado más de 90 meetups y varias ediciones de Pereira Tech Day, conectando el talento local con el ecosistema tech global.

---

## Descargas de marca

- [Logotipo principal (SVG)](/images/brand/pereira-tech-talks-logo.svg)
- [Avatar cuadrado (PNG, 512×512)](/icons/icon-512x512.png)
- [Apple touch icon (PNG, 180×180)](/icons/apple-touch-icon.png)

¿Necesitas un activo adicional? Escríbenos.

---

## Contacto de prensa

Entrevistas, cobertura en sitio, fotografía o solicitudes editoriales:

- Formulario: [Contacto — consulta de prensa](/contact/?topic=other&subject=Consulta%20de%20prensa)
- Correo: [hello@pereiratechtalks.org](mailto:hello@pereiratechtalks.org)

---

## Páginas útiles

- [Sobre la comunidad](/about/)
- [Programas](/verticals/)
- [Pereira Tech Days](/pereira-tech-days/)
- [Patrocinadores](/sponsors/)
- [Contribuyentes](/contributors/)
