...
>
  ...
  <div class="relative mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
    ...
    <ul class="mt-8 divide-y divide-[var(--ptt-border)] rounded-2xl bg-[var(--ptt-bg-elevated)] ring-1 ring-[var(--ptt-border)]">
      {#each items as item, i}
        <li>
          <button
            type="button"
            class="flex w-full items-center justify-between gap-4 px-5 py-4 text-left font-semibold text-[var(--ptt-text)] hover:bg-[var(--ptt-bg)]/50 focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[var(--ptt-primary)]"
            aria-expanded={openIndex === i}
            onclick={() => toggle(i)}
          >
            <span>{tr(item.question, lang)}</span>
            <span class="text-[var(--ptt-accent)]" aria-hidden="true">{openIndex === i ? '−' : '+'}</span>
          </button>
          {#if openIndex === i}
