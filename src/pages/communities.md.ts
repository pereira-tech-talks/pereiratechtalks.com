...
export const GET: APIRoute = async () => {
  ...
  if (past.length > 0) {
    sections.push({ heading: 'Aliadas anteriores', lines: past.map(row) });
  }
  sections.push({
  ...
  sections.push({
    ...
    lines: [
      t.ctaDescription,
      '',
      `- [${t.ctaPrimary}](${SITE_URL}/contact)`,
      `- [${t.ctaSecondary}](${SITE_URL}/channels)`,
    ],
  });
  ...
  const markdown = serializeGenericToMarkdown({
    ...
    metadata: [['Comunidades aliadas', String(active.length)]],
    body: t.heroLead,
    sections,
  });
