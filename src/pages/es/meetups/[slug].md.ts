...
export const GET: APIRoute = ({ props }) => {
  ...
  if (entry.data.speakers.length > 0) {
    sections.push({
      heading: 'Ponentes',
      lines: entry.data.speakers.map((s) => `- [${s}](/es/speakers/${s}.md)`),
    });
  }
  if (entry.data.talks.length > 0) {
    sections.push({
      heading: 'Charlas',
      lines: entry.data.talks.map((t) => `- [${t}](/es/talks/${t}.md)`),
    });
  }
  if (entry.data.verticals.length > 0) {
    sections.push({
      heading: 'Programas',
      lines: entry.data.verticals.map((v) => `- [${v}](/es/verticals/${v}.md)`),
    });
  }
  ...
  const markdown = serializeGenericToMarkdown({
    ...
    description,
    lang,
    canonical: `${SITE_URL}/es/meetups/${slug}`,
    metadata,
    body: entry.body,
    sections,
  });
