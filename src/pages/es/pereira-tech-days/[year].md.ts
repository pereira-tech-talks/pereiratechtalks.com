...
export const GET: APIRoute = ({ props }) => {
  ...
  if (entry.data.keynotes.length > 0) {
    sections.push({
      ...
      lines: entry.data.keynotes.map((s) => `- [${s}](/es/speakers/${s}.md)`),
    });
  }
  if (entry.data.organizers.length > 0) {
    sections.push({
      heading: 'Organizadores',
      lines: entry.data.organizers.map((o) => `- [${o}](/es/contributors)`),
    });
  }
  ...
  const markdown = serializeGenericToMarkdown({
    ...
    description,
    lang,
    canonical: `${SITE_URL}/es/pereira-tech-days/${entry.data.year}`,
    metadata,
    body: entry.body,
    sections,
  });
