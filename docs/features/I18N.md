...
Copy wrappers from `src/pages/en/` and change `lang` value. Each file is 3 lines:
...
- **Page Wrapper Pattern**: Shared `*Page.astro` components in `src/components/pages/` handle `MainLayout` internally; thin wrappers in `src/pages/` and `src/pages/en/`
